...
#include "HttpLibraryRequest.generated.h"

struct HTTPLIBRARY_API FHttpLibraryRequest : IHttpLibraryRequest
{
	FHttpLibraryResponse OnResponse;
...
DECLARE_DYNAMIC_DELEGATE_ThreeParams( FHttpLibraryRequestOnResponse, int32, StatusCode, EHttpLibraryContentType, ContentType, const FString&, Content );
DECLARE_DYNAMIC_DELEGATE_TwoParams( FHttpLibraryRequestOnProgress, int32, BytesSent, int32, BytesReceived );

UCLASS(BlueprintType, meta = (DisplayName = "HTTP Request"))
class HTTPLIBRARY_API UHttpLibraryRequest : public UObject
{
	GENERATED_UCLASS_BODY()

public:
	
	FHttpLibraryRequestOnResponse OnResponse;
	FHttpLibraryRequestOnProgress OnProgress;
	
protected:

	FHttpLibraryRequest Http;

	void TriggerResponse( int32 StatusCode, EHttpLibraryContentType Type, const FString& Content );
