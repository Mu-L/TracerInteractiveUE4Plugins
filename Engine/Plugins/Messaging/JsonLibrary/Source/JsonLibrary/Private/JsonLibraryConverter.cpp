...
TSharedPtr<FJsonValue> ConvertScalarUPropertyToJsonValue(UProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const FJsonLibraryConverter::CustomExportCallback* ExportCb)
{
	...
	else if (UArrayProperty *ArrayProperty = Cast<UArrayProperty>(Property))
	{
		...
		{
			TSharedPtr<FJsonValue> Elem = FJsonLibraryConverter::UPropertyToJsonValue(ArrayProperty->Inner, Helper.GetRawPtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
			if ( Elem.IsValid() )
			{
				// add to the array
				Out.Push(Elem);
	...
	else if ( USetProperty* SetProperty = Cast<USetProperty>(Property) )
	{
		...
		for ( int32 i=0, n=Helper.Num(); n; ++i )
		{
			...
			{
				TSharedPtr<FJsonValue> Elem = FJsonLibraryConverter::UPropertyToJsonValue(SetProperty->ElementProp, Helper.GetElementPtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
				if ( Elem.IsValid() )
				{
					// add to the array
					Out.Push(Elem);
	...
	else if ( UMapProperty* MapProperty = Cast<UMapProperty>(Property) )
	{
		...
		for ( int32 i=0, n = Helper.Num(); n; ++i )
		{
			if ( Helper.IsValidIndex(i) )
			{
				TSharedPtr<FJsonValue> KeyElement = FJsonLibraryConverter::UPropertyToJsonValue(MapProperty->KeyProp, Helper.GetKeyPtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
				TSharedPtr<FJsonValue> ValueElement = FJsonLibraryConverter::UPropertyToJsonValue(MapProperty->ValueProp, Helper.GetValuePtr(i), CheckFlags & ( ~CPF_ParmFlags ), SkipFlags, ExportCb);
	...
	{
		TSharedRef<FJsonObject> Out = MakeShared<FJsonObject>();
		if (FJsonLibraryConverter::UStructToJsonObject(StructProperty->Struct, Value, Out, CheckFlags & (~CPF_ParmFlags), SkipFlags, ExportCb))
		{
			return MakeShared<FJsonValueObject>(Out);
		}
		// fall through to default
...
TSharedPtr<FJsonValue> FJsonLibraryConverter::ObjectJsonCallback(UProperty* Property, const void* Value)
{
	if (UObjectProperty* ObjectProperty = Cast<UObjectProperty>(Property))
	{
		if (!ObjectProperty->HasAnyFlags(RF_Transient)) // We are taking Transient to mean we don't want to serialize to Json either (could make a new flag if nessasary)
		{
			...
			CustomCB.BindStatic(FJsonLibraryConverter::ObjectJsonCallback);

			void** PtrToValuePtr = (void**)Value;

			if (FJsonLibraryConverter::UStructToJsonObject(ObjectProperty->PropertyClass, (*PtrToValuePtr), Out, 0, 0, &CustomCB))
			{
				return MakeShared<FJsonValueObject>(Out);
			}
		}
...
TSharedPtr<FJsonValue> FJsonLibraryConverter::UPropertyToJsonValue(UProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
...
bool FJsonLibraryConverter::UStructToJsonObject(const UStruct* StructDefinition, const void* Struct, TSharedRef<FJsonObject> OutJsonObject, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	return UStructToJsonAttributes(StructDefinition, Struct, OutJsonObject->Values, CheckFlags, SkipFlags, ExportCb);
}

bool FJsonLibraryConverter::UStructToJsonAttributes(const UStruct* StructDefinition, const void* Struct, TMap< FString, TSharedPtr<FJsonValue> >& OutJsonAttributes, int64 CheckFlags, int64 SkipFlags, const CustomExportCallback* ExportCb)
{
	if (SkipFlags == 0)
	{
...
// ---------- UUserDefinedStruct::GetAuthoredNameForField() ----------
		...
		if (bUserStruct)
		{
			...
			if (OriginalName.Len() > MinimalPostfixlen)
			{
				...
				{
					OriginalName = DisplayName.Mid(0, FirstCharToRemove);
				}
			}
		}
// ---------- UUserDefinedStruct::GetAuthoredNameForField() ----------

		FString VariableName = StandardizeCase(OriginalName);
	...
	bool ConvertScalarJsonValueToUPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, UProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags)
	{
		...
		else if (UStructProperty *StructProperty = Cast<UStructProperty>(Property))
		{
			...
			if (JsonValue->Type == EJson::Object)
			{
				...
				{
					UE_LOG(LogJson, Error, TEXT("JsonValueToUProperty - FJsonLibraryConverter::JsonObjectToUStruct failed for property %s"), *Property->GetNameCPP());
					return false;
				}
			}
...
// ---------- UUserDefinedStruct::GetAuthoredNameForField() ----------
			...
			const TSharedPtr<FJsonValue>* JsonValue = JsonAttributes.Find(OriginalName);
			if (!JsonValue)
			{
...
bool FJsonLibraryConverter::JsonValueToUProperty(const TSharedPtr<FJsonValue>& JsonValue, UProperty* Property, void* OutValue, int64 CheckFlags, int64 SkipFlags)
{
	return JsonValueToUPropertyWithContainer(JsonValue, Property, OutValue, nullptr, nullptr, CheckFlags, SkipFlags);
}

bool FJsonLibraryConverter::JsonObjectToUStruct(const TSharedRef<FJsonObject>& JsonObject, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags)
{
	return JsonAttributesToUStruct(JsonObject->Values, StructDefinition, OutStruct, CheckFlags, SkipFlags);
}

bool FJsonLibraryConverter::JsonAttributesToUStruct(const TMap< FString, TSharedPtr<FJsonValue> >& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags)
{
	...
	if (JsonObject.IsValid())
	{
		for (const auto& It : JsonObject->Values)
		{
			...
			case EJson::Object:
			{
				// localized string
				FText TextOut;
				if (FJsonLibraryConverter::GetTextFromObject(It.Value->AsObject().ToSharedRef(), TextOut))
				{
