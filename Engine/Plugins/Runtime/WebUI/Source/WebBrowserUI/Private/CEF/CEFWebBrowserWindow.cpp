...
EWebInterfaceBrowserDocumentState FCEFWebBrowserWindow::GetDocumentLoadingState() const
{
...
bool FCEFWebBrowserWindow::OnJSDialog(CefJSDialogHandler::JSDialogType DialogType, const CefString& MessageText, const CefString& DefaultPromptText, CefRefPtr<CefJSDialogCallback> Callback, bool& OutSuppressMessage)
{
	...
	{
		TSharedPtr<IWebInterfaceBrowserDialog> Dialog(new FCEFWebBrowserDialog(DialogType, MessageText, DefaultPromptText, Callback));
...
bool FCEFWebBrowserWindow::OnBeforeUnloadDialog(const CefString& MessageText, bool IsReload, CefRefPtr<CefJSDialogCallback> Callback)
{
	...
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IWebInterfaceBrowserDialog> Dialog(new FCEFWebBrowserDialog(MessageText, IsReload, Callback));
		EWebInterfaceBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IWebInterfaceBrowserDialog>(Dialog));
		switch (EventResponse)
		{
		case EWebInterfaceBrowserDialogEventResponse::Handled:
...
void FCEFWebBrowserWindow::UpdateDragRegions(const TArray<FWebInterfaceBrowserDragRegion>& Regions)
{
	DragRegions = Regions;
}
