// Engine/Source/Runtime/WebBrowser/Private/MobileJS/MobileJSStructDeserializerBackend.cpp
		...
		case EJsonNotation::String:
		{
			if (Property->IsA<UStructProperty>())
			{
				...
				if ( StructProperty->Struct == FWebInterfaceJSFunction::StaticStruct())
				{
					...
					return SetPropertyValue<UStructProperty, FWebInterfaceJSFunction>(Property, Outer, Data, ArrayIndex, CallbackObject);
				}
			}
		}
		break;
