...
namespace NativeFuncs
{
	...
	bool WriteJsParam(FNativeJSScriptingRef Scripting, FJsonWriterRef Writer, const KeyType& Key, FWebInterfaceJSParam& Param)
	{
		switch (Param.Tag)
		{
...
void FNativeJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FWebInterfaceJSParam Arguments[], bool bIsError)
{
	if (!IsValid())
	{
		return;
...
bool FNativeJSScripting::HandleExecuteUObjectMethodMessage(const TArray<FString>& MessageArgs)
{
	...
	if ( ! PromiseParam ) // If PromiseParam is set, we assume that the UFunction will ensure it is called with the result
	{
		if ( ReturnParam )
		{
			...
			ReturnPolicies.PropertyFilter = [&ReturnParam](const UProperty* CandidateProperty, const UProperty* ParentProperty)
			{
